...
yaml_file = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "host.yaml")
...
def get_host_name_list(data):
    return str(fun[data.get("type", "default")](data.get("host_name")))
...
def render_params():
    ...
    return {k: get_host_name_list(v) for k, v in data.items()}
