...
OUTPUT_DIR_TAR=${OUTPUT_DIR}/${MODULE_NAME}.tar

# 必须先将要打包的东西放到output/<模块名>目录下
if not test -d "${OUTPUT_DIR}"; then
  echo 'output dir does not exists!'
  exit 1
fi

cd output/${MODULE_NAME}
tar czf ${MODULE_NAME}.tar ./*
cd -

MD5=$(md5sum ${OUTPUT_DIR_TAR} | awk '{print $1}')
...
# 上传
rsync -vz ${OUTPUT_DIR_TAR} ${RSYNC_PREFIX}/${RSYNC_PATH}
# 标记上传成功
...
# 新制品库
curl -f -v -u ${NEXUS_USER}:${NEXUS_PASSWD} --upload-file ${OUTPUT_DIR_TAR} https://nexus-deliver-internal.sensorsdata.cn/repository/deliver-test/${CUSTOMER_NAME}/${MODULE_NAME}/${VERSION}/${MODULE_NAME}.tar
